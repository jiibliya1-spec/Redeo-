...
export function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Step 1: just show confirmation screen
    if (step === 1) {
      // Basic validation
    ...
    try {
      ...
      // Refresh auth state in the store (user is now signed in)
      await initAuth();
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#A0A0A0] mt-1">
            {step === 1 ? 'Join WansniAuto today' : 'Review and confirm'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {step === 1 && (
            <>
                  ...
                  >
                    <User
                      className={`w-6 h-6 mx-auto mb-2 ${
                        role === r ? 'text-[#FF6B00]' : 'text-[#A0A0A0]'
                      }`}
                    />
              ...
              <div>
                <Label className="text-sm text-[#A0A0A0] mb-2 block">Full name</Label>
                <Input
                  value={name}
                  onChange={e => setName(e.target.value)}
                  placeholder="Your name"
                  className="bg-[#1B1F27] border-white/10 text-white placeholder:text-[#A0A0A0]/40 h-12 rounded-xl"
                  required
                />
              </div>
              <div>
            ...
            <>
              ...
              <label className="flex items-start gap-3 cursor-pointer p-4 rounded-xl border border-white/5 hover:border-white/10">
                ...
                <span className="text-sm text-[#A0A0A0]">
                  I agree to the{' '}
                  <span className="text-[#FF6B00]">Terms of Service</span> and{' '}
                  <span className="text-[#FF6B00]">Privacy Policy</span>. I
