...
import { Car, Eye, EyeOff, Loader2, User } from 'lucide-react';

export function RegisterPage() {
  const navigate = useNavigate();
  // Registration handles login automatically
            ...
            <>
              <div className="grid grid-cols-2 gap-3 mb-2">
                {(['passenger', 'driver'] as const).map(r => (
                  <button key={r} type="button" onClick={() => setRole(r)} className={`p-4 rounded-xl border text-center transition-all ${role === r ? 'border-[#FF6B00]/50 bg-[#FF6B00]/5' : 'border-white/5 hover:border-white/10'}`}>
                    <User className={`w-6 h-6 mx-auto mb-2 ${role === r ? 'text-[#FF6B00]' : 'text-[#A0A0A0]'}`} />
              ...
              <div><Label className="text-sm text-[#A0A0A0] mb-2 block">Password</Label>
                <div className="relative">
                  <Input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 8 characters" className="bg-[#1B1F27] border-white/10 text-white placeholder:text-[#A0A0A0]/40 h-12 rounded-xl pr-10" required minLength={8} />
          ...
          {step === 2 && (
            <>
              ...
              <label className="flex items-start gap-3 cursor-pointer p-4 rounded-xl border border-white/5 hover:border-white/10">
                ...
                <span className="text-sm text-[#A0A0A0]">I agree to the <span className="text-[#FF6B00]">Terms of Service</span> and <span className="text-[#FF6B00]">Privacy Policy</span>. I confirm I am at least 18 years old.</span>
              </label>
            </>
          )}
